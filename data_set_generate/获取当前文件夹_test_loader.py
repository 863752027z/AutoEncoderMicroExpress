...
def test(test_loader, seq, encoder_moudle, cell_moudle):
    for idx, (data, label) in enumerate(test_loader):
        ...
        if idx == seq-1:
            ...
            feature = torch.cat((temp_target, cell_out), 1)
        if idx >= seq:
            h = torch.zeros(seq-1, 256).to(device)
            c = torch.zeros(seq-1, 256).to(device)
            Quad = Quad[1:, :, :, :]
...
#cell_moudle = torch.load(moudle_path + 'cell_modle.pkl')
encoder_moudle = torch.load(moudle_path + 'encoder_modle.pkl')
#print(cell_moudle)
print(encoder_moudle)
exit(0)
...
for i in range(len(test_list1)):
    ...
    test(temp_loader, seq, encoder_moudle, cell_moudle)
